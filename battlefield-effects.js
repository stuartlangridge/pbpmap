...
class BattlefieldEffects extends HTMLElement {
    constructor() {
        ...
        async function addEffect(details) {
            ...
            "0ft,5ft,10ft,15ft,20ft,30ft,40ft".split(",").forEach(o => {
        ...
        this.EFFECTS = {
            ...
            FIRE_old: (ctx, mainColour, radiusSquares, opacity, cxp, cyp, squareSize, done) => {
                ...
                // feed it orange, you get red and yellow as dark and light
                const midColourHSL = HexToHSL(mainColour);
                const darkColourHSL = {
                    h: midColourHSL.h - 30,
                    s: midColourHSL.s,
                    l: midColourHSL.l
                }
                if (darkColourHSL.h < 0) darkColourHSL.h = 360 + darkColourHSL.h;
                const lightColourHSL = {
                    h: (midColourHSL.h + 30) % 360,
                    s: midColourHSL.s,
                    l: midColourHSL.l
                }

                const COLS = [
                    HSLToHex(darkColourHSL) + opacityHex,
                    HSLToHex(midColourHSL) + opacityHex,
                    HSLToHex(lightColourHSL) + opacityHex 
                ];
    ...
    async renderEffects(ctx, effects, overallAlpha, overrideGridSettings, done) {
        ...
        async function doNext() {
            ...
            let size = {"0ft": 0, "5ft": 1, "10ft": 2, "15ft": 3, "20ft": 4, "30ft": 6, "40ft": 8}[ne.size];
